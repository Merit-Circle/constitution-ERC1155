...
import { MeritNFT, MeritNFT__factory, WhitelistedNFTSale, WhitelistedNFTSale__factory } from "../src/types";
...
describe("WhiteListedNFTSale", function () {
  before(async () => {
    ...
    NFT = await new MeritNFT__factory(deployer).deploy(NAME, SYMBOL, BASE_TOKEN_URI);
  ...
  describe("buy", async () => {
    ...
    it("Minting more than user cap should mint up to the user cap", async () => {
      ...
      const proof = saleMerkleTree.getProofByAddress(signers[0].address);
      const payerETHBalanceBefore = await signers[0].getBalance("latest");
      const buyTx = await sale
        .connect(signers[0])
        .buy(amountToMint, account1.address, proof, { value: PRICE.mul(USER_CAP) });
      const receipt = await buyTx.wait();
    ...
    it("Buying before the sale has started should fail", async () => {
      ...
      const proof = saleMerkleTree.getProofByAddress(signers[0].address);
      await expect(tempSale.connect(signers[0]).buy(USER_CAP, account1.address, proof)).to.be.revertedWith(
        "SaleNotStartedError()",
      );
    });
    ...
    it("Buying after sale has ended should fail", async () => {
      ...
      const proof = saleMerkleTree.getProofByAddress(signers[0].address);
      await expect(
        sale.connect(signers[0]).buy(USER_CAP, account1.address, proof, { value: parseEther("1") }),
      ).to.be.revertedWith("SaleEndedError()");
    });

    it("Not sending sufficient ETH with the buy contract call should fail", async () => {
      const proof = saleMerkleTree.getProofByAddress(signers[0].address);
      await expect(
        sale.connect(signers[0]).buy(USER_CAP, account1.address, proof, { value: parseEther("0.05") }),
      ).to.be.revertedWith("InsufficientETHError()");
    });
    ...
    it("Submitting a false merkle proof should fail", async () => {
      ...
      await expect(
        sale.connect(signers[0]).buy(USER_CAP, account1.address, proof, { value: parseEther("1") }),
      ).to.be.revertedWith("MerkleProofVerificationError()");
    });
    ...
    it("Trying to buy more while already have bought up to cap should fail", async () => {
      ...
      await expect(
        sale.connect(signers[0]).buy(USER_CAP, account1.address, proof, { value: parseEther("1") }),
      ).to.be.revertedWith("ZeroMintError()");
    });
  });
