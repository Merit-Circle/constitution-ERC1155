...
describe("MeritNFTDropFactory", function () {
  ...
  describe("deployNFT", async () => {
    it("Deploying an immutable NFT should work", async () => {
      ...
      const nftAddress = await factory.callStatic.deployNFT(NAME, SYMBOL, BASE_TOKEN_URI, MERKLE_ROOT, IPFS_HASH, true);

      // actually deploy the contract
      const nftDeployTx = factory.deployNFT(NAME, SYMBOL, BASE_TOKEN_URI, MERKLE_ROOT, IPFS_HASH, true);
    ...
    it("Deploying a mutable NFT should work", async () => {
      ...
      // actually deploy the contract
      const nftDeployTx = factory.deployNFT(NAME, SYMBOL, BASE_TOKEN_URI, MERKLE_ROOT, IPFS_HASH, false);
  ...
  describe("claim", async () => {
    ...
    it("Claiming twice should fail", async () => {
      ...
      await expect(factory.claim(NFT.address, 0, signers[0].address, proof)).to.be.revertedWith(
